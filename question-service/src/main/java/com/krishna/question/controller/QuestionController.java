package com.krishna.question.controller;
...
import com.krishna.question.utility.QuestionLoggerMessage;
...
public class QuestionController {
    ...
    @DeleteMapping("/deletebyquiz/{quizId}")
    public ResponseEntity<SuccessResponse> deleteQuestionsByQuizId(@PathVariable int quizId) {
        
            boolean deletionStatus = questionService.deleteQuestionsByQuizId(quizId);
            if (deletionStatus) {
                return ResponseEntity.ok(new SuccessResponse(HttpStatus.OK.value(), "Questions deleted successfully for the quiz."));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new SuccessResponse(HttpStatus.NOT_FOUND.value(), "No questions found for the given quiz ID."));
            }
    ...
    public final ResponseEntity<List<QuestionDto>> getAllQuestionByQuizId(
    		  @PathVariable final int quizId) {
        LOGGER.info(QuestionLoggerMessage.GET_ALL_QUESTIONS_REQUEST);
        List<QuestionDto> questions = questionService.getQuestionByQuizId(quizId);
        LOGGER.info(
              QuestionLoggerMessage.LIST_OF_QUESTIONS_RETRIEVED_SUCCESSFULLY);
        return new ResponseEntity<>(questions, HttpStatus.OK);
    }
